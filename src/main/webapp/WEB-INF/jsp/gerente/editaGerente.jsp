...
<form:form commandName="gerente" method="POST" action="/sindico/criaGerente">
	<table style="margin: 10px;">
		...
		<tr>
			...
			<td><form:input path="email" value="${gerente.email }" /></td>
		</tr>
		<tr>
			<td><strong>Telefone:</strong></td>
			<td><form:input path="telefone" value="${gerente.telefone }"/></td>
		</tr>
		<tr>
			<td>Celular:</td>
			<td><form:input path="celular" value="${gerente.celular }"/></td>
		</tr>
		<tr>
			<td><strong>Senha</strong></td>
			<td><form:password path="senha" value="${gerente.senha }" /></td>
		</tr>
		<tr>
			<td><strong>Recebe email de Marketing?</strong></td>
			<td>
				<form:select path="recebeEmailMkt">
					<c:if test="${gerente.recebeEmailMkt == true }">
						<form:option value="true" selected="true">Recebe</form:option>
						<form:option value="false">Não Recebe</form:option>
					</c:if>
					<c:if test="${gerente.recebeEmailMkt == false }">
						<form:option value="true">Recebe</form:option>
						<form:option value="false" selected="true">Não Recebe</form:option>
					</c:if>
				</form:select>
			</td>
		</tr>
